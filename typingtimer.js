...
function doneTyping2 () {
  async function userAction2() {
    const response = await fetch(`https://translation2.jacebillingsley.repl.co/${lang2}/${lang1}?content=${document.getElementById('secondary-input').value}`);
...
function doneTyping () {
  async function userAction() {
    const response = await fetch(`https://translation2.jacebillingsley.repl.co/${lang1}/${lang2}?content=${document.getElementById('primary-input').value}`);
    const myJson = await response.json(); 
