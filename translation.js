async function translate(section) {
  ...
  const response = await fetch(`https://translation2.jacebillingsley.repl.co/${temp1}/${temp2}?content=${document.getElementById(section + '-input').value}`);
  let myJson = await response.json(); 
  document.getElementById(section + '-clarification').value = myJson.clarification;
}

async function changeLang(section, language) {
  const response = await fetch(`https://translation2.jacebillingsley.repl.co/get${section}/${language}`);
